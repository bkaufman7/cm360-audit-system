...
function getStagingMode_() {
	try {
		// Default to 'Y' to prevent accidental production sends when property is missing
		const raw = PropertiesService.getScriptProperties().getProperty('STAGING_MODE');
		const v = String((raw === null || typeof raw === 'undefined') ? 'Y' : raw).trim().toUpperCase();
		return v === 'Y' ? 'Y' : 'N';
	} catch (e) {
		// If anything goes wrong, fall back to staging
		return 'Y';
	}
}
...
function safeSendEmail(opts = {}, context = '') {
	...
	let plainBody = '';
	let mailOpts = {};
	// Always include this address in every outbound (production) email unless staging suppresses
	const ALWAYS_INCLUDE_EMAIL = 'bkaufman@horizonmedia.com';
	try {
		...
		// Production path: ensure ALWAYS_INCLUDE_EMAIL is present (prefer CC so main "to" semantics remain intact)
		if (ALWAYS_INCLUDE_EMAIL) {
			...
			const canonical = (x) => String(x || '').trim().toLowerCase();
			const targetCanon = canonical(ALWAYS_INCLUDE_EMAIL);
			const inTo = normalizeList(to).some(e => canonical(e) === targetCanon);
			const inCc = normalizeList(mailOpts.cc).some(e => canonical(e) === targetCanon);
			const inBcc = normalizeList(mailOpts.bcc).some(e => canonical(e) === targetCanon);
			if (!inTo && !inCc && !inBcc) {
				const newCc = normalizeList(mailOpts.cc);
				newCc.push(ALWAYS_INCLUDE_EMAIL);
				// Deduplicate & rejoin
				mailOpts.cc = Array.from(new Set(newCc.map(x => x && x.trim()).filter(Boolean))).join(', ');
				Logger.log(`[EMAIL] Added universal recipient ${ALWAYS_INCLUDE_EMAIL} via CC (${context})`);
			}
		}
		...
		try {
			...
			// Ensure universal recipient also receives failure alerts (if not staging suppression)
			let alertRecipients = ADMIN_EMAIL;
			if (ALWAYS_INCLUDE_EMAIL && ALWAYS_INCLUDE_EMAIL !== ADMIN_EMAIL) {
				alertRecipients = `${ADMIN_EMAIL}, ${ALWAYS_INCLUDE_EMAIL}`;
			}
			GmailApp.sendEmail(alertRecipients, fallbackSubject, fallbackBody);
			Logger.log(`[EMAIL] Failure alert sent to ${alertRecipients}`);
		} catch (notifyErr) {
...
function getAuditConfigBatches(batchSize = BATCH_SIZE) {
	...
	// Any config listed in DEDICATED_BATCH_CONFIGS is placed in its own batch
	const dedicatedSet = new Set((DEDICATED_BATCH_CONFIGS || []).map(n => String(n || '').trim()).filter(Boolean));
	const dedicatedBatches = [];
	const remaining = [];

	Logger.log(`[Batch Assignment] Total configs: ${configs.length}`);
	Logger.log(`[Batch Assignment] Dedicated configs (isolated): ${Array.from(dedicatedSet).join(', ') || 'none'}`);

	for (const cfg of configs) {
		if (!cfg) continue;
		const name = String(cfg.name || '').trim();
		if (name && dedicatedSet.has(name)) {
			Logger.log(`[Batch Assignment] Isolating ${name} into dedicated batch`);
			dedicatedBatches.push([cfg]);
		} else {
			remaining.push(cfg);
		}
	}
	...
	const batches = [];
	// Place dedicated batches first to run early and avoid timeouts
	dedicatedBatches.forEach(b => batches.push(b));

	Logger.log(`[Batch Assignment] Created ${dedicatedBatches.length} dedicated batch(es)`);
	Logger.log(`[Batch Assignment] Remaining configs to batch: ${remaining.length}`);

	for (let i = 0; i < remaining.length; i += batchSize) {
		batches.push(remaining.slice(i, i + batchSize));
	}

	Logger.log(`[Batch Assignment] Total batches: ${batches.length}`);
	batches.forEach((batch, idx) => {
		Logger.log(`[Batch Assignment] Batch ${idx + 1}: ${batch.map(c => c.name).join(', ')}`);
	});
...
/**
 * Debug function to verify NEXTSD01 (or any dedicated config) is properly isolated
 * Run this from Apps Script editor to check batch assignments
 */
function verifyNEXTSD01Isolation() {
	try {
		Logger.log('=== NEXTSD01 ISOLATION VERIFICATION ===\n');
		
		// Check Script Property
		const dedicatedProp = PropertiesService.getScriptProperties().getProperty('DEDICATED_BATCH_CONFIGS');
		Logger.log(`Script Property DEDICATED_BATCH_CONFIGS: ${dedicatedProp || '(not set - using default)'}`);
		
		// Check constant value
		Logger.log(`DEDICATED_BATCH_CONFIGS constant: ${JSON.stringify(DEDICATED_BATCH_CONFIGS)}`);
		
		// Check batch assignments
		const batches = getAuditConfigBatches(BATCH_SIZE);
		Logger.log(`\nTotal batches: ${batches.length}`);
		
		let foundNEXTSD01 = false;
		let nextsd01BatchIndex = -1;
		let nextsd01BatchSize = 0;
		
		batches.forEach((batch, idx) => {
			const configNames = batch.map(c => c.name).join(', ');
			Logger.log(`\nBatch ${idx + 1} (${batch.length} config${batch.length === 1 ? '' : 's'}): ${configNames}`);
			
			if (batch.some(c => c.name === 'NEXTSD01')) {
				foundNEXTSD01 = true;
				nextsd01BatchIndex = idx + 1;
				nextsd01BatchSize = batch.length;
				Logger.log(`  ⚠️ NEXTSD01 found here!`);
			}
		});
		
		Logger.log('\n=== VERIFICATION RESULTS ===');
		if (!foundNEXTSD01) {
			Logger.log('❌ NEXTSD01 not found in any batch - check Recipients sheet');
			SpreadsheetApp.getUi().alert('NEXTSD01 not found in any batch.\nCheck that it exists and is Active in Audit Recipients sheet.');
			return 'NEXTSD01 not found';
		}
		
		if (nextsd01BatchSize === 1) {
			Logger.log(`✅ NEXTSD01 is properly isolated in Batch ${nextsd01BatchIndex} (alone)`);
			SpreadsheetApp.getUi().alert(`✅ SUCCESS!\n\nNEXTSD01 is properly isolated in Batch ${nextsd01BatchIndex}.\nIt will run independently without timing out.`);
			return 'NEXTSD01 properly isolated';
		} else {
			Logger.log(`❌ NEXTSD01 is in Batch ${nextsd01BatchIndex} with ${nextsd01BatchSize - 1} other config(s)`);
			Logger.log(`   This will cause timeouts!`);
			SpreadsheetApp.getUi().alert(`❌ PROBLEM DETECTED!\n\nNEXTSD01 is sharing Batch ${nextsd01BatchIndex} with ${nextsd01BatchSize - 1} other config(s).\n\nThis will cause timeouts. Check View > Logs for details.`);
			return `NEXTSD01 not isolated - sharing batch with ${nextsd01BatchSize - 1} other config(s)`;
		}
		
	} catch (error) {
		Logger.log(`❌ Error verifying isolation: ${error.message}`);
...
function getAdminControlsHelpItems() {
	const items = [
		...
		{ label: '🔍  Verify NEXTSD01 Isolation', fn: 'verifyNEXTSD01Isolation', desc: 'Checks if NEXTSD01 (or other dedicated configs) is properly isolated in its own batch to prevent timeouts.' },
...
function runWRI01Audit() { runDailyAuditByName('WRI01'); }
